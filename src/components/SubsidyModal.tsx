...
interface Props {
  ...
  onConfirm: (selections: MemberSubsidySelection[]) => void
  onClose: () => void
}

export function SubsidyModal({ open, subsidyTypes, currentSelections, onConfirm, onClose }: Props) {
  const [selections, setSelections] = useState<MemberSubsidySelection[]>(() =>
    currentSelections.map(s => ({ ...s })),
  )
  ...
  const getLevelGold = (typeId: string, levelName: string) => {
    const st = subsidyTypes.find(t => t.id === typeId)
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent className="max-w-sm">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button size="sm" onClick={handleConfirm}>保存登记</Button>
        </div>
      </DialogContent>
    </Dialog>
  )
